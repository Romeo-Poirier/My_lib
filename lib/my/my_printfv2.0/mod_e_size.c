...
static long long int unite(long long int nb, int precision)
{
    ...
    full_t = get_precision_rev(full_t, (my_llilen(full_t) - 1));
    return full_t;
}

static long long int res_e(long long int nb)
{
    long long int e = my_llilen(nb) - 1;
    int len = 0;
...
static long long int nbr_d(long long int f, long long int e, int p)
{
    ...
    if (e == 0)
        return res;
    while (my_llilen(f) != my_llilen(e))
        e *= 10;
    res = f - e;
    if (res == 0)
        return res;
    while (my_llilen(res) != p)
        res /= 10;
    return res;
}
